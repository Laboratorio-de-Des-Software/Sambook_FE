...
function ListaAutoresSE() {
  ...
  const handleDragEnd = async (e: DropResult) => {
    ...
    updatedAutores.forEach((autor) => {
      const docRef = firebase.firestore().collection('autores_SE').doc(autor.id)
        ...
        if (doc.exists) {
          ...
          if (docData) {
            ...
            `,
              ...
              preConfirm: () => {
                const novoNomeAutSInput = document.getElementById(
                  'nomeAutor'
                ) as HTMLInputElement
                const novoNome = novoNomeAutSInput.value

                if (!novoNome) {
                  if (!novoNome) {
                    novoNomeAutSInput.classList.add('borda')
                  } else {
                    novoNomeAutSInput.classList.remove('borda')
                  }
        ...
        <div className="col-md-6 mt-3">
          <div className="input-group mb-3">
            <input
              ...
              placeholder="Adicionar Autor de Samba Enredo"
